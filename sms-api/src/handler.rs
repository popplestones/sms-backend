...
use serde_json::json;
use sms_core::models::{SmsSubmission, job::OutgoingJob};
...
async fn is_queue_overloaded(
    con: &mut redis::aio::MultiplexedConnection,
) -> Result<bool, RedisError> {
    let len: usize = con.llen("sms_queue").await?;
    let threshold: usize = std::env::var("MAX_QUEUE_LENGTH")
        .ok()
